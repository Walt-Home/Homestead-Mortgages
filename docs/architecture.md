...
| **Address**                                             | Google Places API v1 over CoreLogic Property API v2                  | Places answers autocomplete (`PROPERTY_DATA_PROVIDER=google_places`); CoreLogic the record and the AVM (`PROPERTY_RECORDS_PROVIDER=corelogic`). ⚠ Flood stays fixture; nobody counts the trial budget                 |
...
apps/console ───► (nothing of ours)  the ops console: its own tokens, Doug's console API over HTTP through apps/api
...
| `packages/connectors`   | Fourteen ports, fixture adapters for all fourteen, seven real ones (three partial), and the authorization guard                                                                                                                                                                                                                                                                                                                              |
...
| `apps/console`          | The ops console, ours: React + Vite over Doug's console API, with a design system of its own for an operator workspace (`src/theme.css`); served by the API container on the servicing hostname under `/console`, which forwards `/console/api/*` to his `/ops/api/*`                                                                                                                                                                        |
...
Fourteen ports in one file. Seven have a real adapter (three of them partial); seven are fixture-only.
...
| `servicing`                                                    | Doug's runtime, `apps/servicing`, over its `/v1` | Keyed on the servicer's loan number, unguarded; reads his events and the two tools his bus allows a system actor. Fixture answers his engine's verdicts for the sample book |
