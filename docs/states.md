...
| **Co-borrower**       | Named by the applicant — name, email, whether they will live in the home — as a PROVISIONAL party on the applicant's application, and listed as invited rather than as a borrower. Completes their own profile and gives their own permissions in their own session; until then the application waits. Claimed the Grander way — a single-use link emailed to them, good for seven days, its token held nowhere but the email — and the named party merges into the one their Google sign-in creates; the trigger admits no other edge. Never an email match: the address is where the link went, not who may follow it |
...
| Loans and servicing                    | Built, and a partner's tape writes it — `docs/loan-lifecycle.md`. The claim is a token the servicer delivers, or the tape desk mails |
