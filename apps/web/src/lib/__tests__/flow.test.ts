/**
 * The eleven names the server sends, against the map that has to know them all.
 *
 ...
 * the stepper's dots unlinkable on every screen.
 *
 ...
 * binds both routes to the same names.
 */
...
describe("every stage the server can send", () => {
  it("resumes to one of the five screens", () => {
  ...
  it("answers how far the borrower has got, rather than nowhere", () => {
    // −1 is what this returned for every one of them, and −1 is not a step
    // index: it is the answer `findIndex` gives when the stage was never in
    // the map.
    for (const stage of FLOW_STAGES) {
