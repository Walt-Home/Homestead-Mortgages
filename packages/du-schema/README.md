...
They are in the tree under a decision recorded in `docs/du-readiness.md`: build
as though the licenses and the agency agreement are in place, on the
understanding that no real borrower and no real loan goes through this system
until they actually are. Git keeps a file forever once it lands, which is why
that is written here rather than carried in somebody's head.
...
```
xmllint --noout --schema packages/du-schema/xsd/DU_Wrapper_3.4.0_B324.xsd <file>
```
...
`src/__tests__/schema.test.ts` asserts both halves, line for line: the eighteen
...
maximum is a CHECK or a trigger, and `docs/entities.md` gives the reason in one
line: an invariant held by a trigger survives a new route written by somebody
who never read this file, and an invariant held by application code does not.
The gate on a submission is the preflight in `packages/du`, not this.

## Using it

```ts
import { samplePaths, xmllintErrors } from "@hm/du-schema";
...
`xmllint` ships with macOS and with the Linux images this repo builds on. If it
