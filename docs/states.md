...
| **Co-borrower**       | Named by the applicant — name, email, whether they will live in the home — as a PROVISIONAL party on the applicant's application, and listed as invited rather than as a borrower. Completes their own profile and gives their own permissions in their own session; until then the application waits. Claimed the Grander way, by a signed single-use token delivered by email, and the named party merges into the one their sign-in creates — the trigger admits no other edge |
